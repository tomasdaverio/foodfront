...
export const getRecipeByName = (recipe) => {
   return async (dispatch) => {
    try{
        const response = await fetch(`https://foodback-1poi.onrender.com/recipes?name=${recipe}`) ;
...
export const getRecipeById = (id) => {
    return async (dispatch) => {
     try{
         const response = await fetch(`https://foodback-1poi.onrender.com/recipes/${id}`) ;
         const pay = await response.json() ;
         if(pay.error) {
            console.log(pay.status,pay.error) ;
...
export const addRecipe = (data) => {
    return async (dispatch) => {
     try{
         const response = await fetch('https://foodback-1poi.onrender.com/recipes', {
         method: 'POST', 
